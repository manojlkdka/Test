...
class InsPartnerLedger(models.TransientModel):
    ...
    nepali_date_to_pl = fields.Char('Date (B.S.)', compute="_get_nepalidate_to_pl", store=True)
    nepali_date_from_pl = fields.Char('Date (B.S.)', compute="_get_nepalidate_from_pl", store=True)

    @api.depends('date_to')
    def _get_nepalidate_to_pl(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_to:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_to), dt_format)
                jrnl.update({'nepali_date_to_pl': dt})
    
    @api.depends('date_from')
    def _get_nepalidate_from_pl(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_from:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_from), dt_format)
                jrnl.update({'nepali_date_from_pl': dt})
    ...
    def process_filters(self):
        ...
        if data.get('date_from_pl', False):
            filters['date_from_pl'] = data.get('date_from_pl')
        if data.get('date_to_pl', False):
            filters['date_to_pl'] = data.get('date_to_pl')
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'date_from_pl': self.nepali_date_from_pl,
            'date_to_pl': self.nepali_date_to_pl,
