...
class InsFinancialReport(models.TransientModel):
    ...
    def get_report_values(self):
        ...
        data['form'] = self.read(
            ['date_from','nepali_date_from_fr','enable_filter', 'debit_credit', 'date_to', 'date_range','nepali_date_to_fr',
        ...
        date_from, date_to,date_from_fr,date_to_fr = False, False,False,False
        used_context = {}
        used_context['date_from'] = self.date_from or False
        used_context['date_to'] = self.date_to or False
        used_context['date_from_fr'] = self.nepali_date_from_fr or False
        used_context['date_to_fr'] = self.nepali_date_to_fr or False
    ...
    nepali_date_to_fr = fields.Char('Date (B.S.)', compute="_get_nepalidate_to_fr", store=True)
    nepali_date_from_fr = fields.Char('Date (B.S.)', compute="_get_nepalidate_from_fr", store=True)

    @api.depends('date_to')
    def _get_nepalidate_to_fr(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_to:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_to), dt_format)
                jrnl.update({'nepali_date_to_fr': dt})
    
    @api.depends('date_from')
    def _get_nepalidate_from_fr(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_from:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_from), dt_format)
                jrnl.update({'nepali_date_from_fr': dt})
