...
class InsTrialBalance(models.TransientModel):
    ...
    nepali_date_to_tb = fields.Char('Date (B.S.)', compute="_get_nepalidate_to_tb", store=True)
    nepali_date_from_tb = fields.Char('Date (B.S.)', compute="_get_nepalidate_from_tb", store=True)

    @api.depends('date_to')
    def _get_nepalidate_to_tb(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_to:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_to), dt_format)
                jrnl.update({'nepali_date_to_tb': dt})
    
    @api.depends('date_from')
    def _get_nepalidate_from_tb(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_from:
                np_dt_abs = self.env['np.date']
                dt = np_dt_abs.get_nepalidate(str(jrnl.date_from), dt_format)
                jrnl.update({'nepali_date_from_tb': dt})
    ...
    def process_filters(self, data):
        ...
        if data.get('date_from_tb',False):
            filters['date_from_tb'] = data.get('date_from_tb')
        if data.get('date_to_tb', False):
            filters['date_to_tb'] = data.get('date_to_tb')
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'date_from_tb': self.nepali_date_from_tb,
            'date_to_tb': self.nepali_date_to_tb,
