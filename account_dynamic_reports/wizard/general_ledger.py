...
class InsGeneralLedger(models.TransientModel):
    ...
    nepali_date_to_gl = fields.Char('Date (B.S.)', compute="_get_nepalidate_to_gl", store=True)
    nepali_date_from_gl = fields.Char('Date (B.S.)', compute="_get_nepalidate_from_gl", store=True)

    @api.depends('date_to')
    def _get_nepalidate_to_gl(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_to:
    ...
    def _get_nepalidate_from_gl(self):
        dt_format = '%Y-%m-%d'
        for jrnl in self:
            if jrnl.date_from:
                np_dt_abs = self.env['np.date']
    ...
    def process_filters(self):
        ...
        if data.get('date_from_gl', False):
            filters['date_from_gl'] = data.get('date_from_gl')
        if data.get('date_to_gl', False):
            filters['date_to_gl'] = data.get('date_to_gl')
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'date_from_gl': self.nepali_date_from_gl,
            'date_to_gl': self.nepali_date_to_gl,
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            ...
            '.action_print_general_ledger').report_action(
